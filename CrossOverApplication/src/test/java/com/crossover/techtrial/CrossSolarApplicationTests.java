...
import main.java.com.crossover.techtrial.service.Pageable;
...
public class CrossSolarApplicationTests {
  ...
  public void contextLoads3() throws Exception{
			...
			RequestBuilder requestBuilder = MockMvcRequestBuilders.get(
					"/hourlyElectricity/HourlyElectricity1").accept(
					MediaType.APPLICATION_JSON);

			MvcResult result = mockMvc.perform(requestBuilder).andReturn();

			System.out.println(result.getResponse());
			String expected = "{success:success1}";
  ...
  public void contextLoads4() throws Exception{
			...
			Mockito.when(
					hourlyElectricityService.getAllHourlyElectricityByPanelId(Mockito.anyString(),
							Mockito.anyString())).thenReturn(panelId, "");

			RequestBuilder requestBuilder = MockMvcRequestBuilders.get(
					"/hourlyElectricity/HourlyElectricity1/panelId/PanelID1").accept(
					MediaType.APPLICATION_JSON);

			MvcResult result = mockMvc.perform(requestBuilder).andReturn();

			System.out.println(result.getResponse());
			String expected = "{id:id1,generatedElectricity:generatedElectricity1,readingAt:readingAt1}";
